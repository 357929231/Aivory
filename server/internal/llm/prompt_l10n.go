...
//   - tool NAMES the model must call verbatim: aivory_web_search / python_execute /
//     image_generate / use_skill / save_memory
...
var promptL10nTable = map[string]promptL10n{
	"en": {
		...
		dateGrounding:      "Today is %s. Treat \"today\", \"now\", \"latest\", and \"recent\" — including dates in web search queries — as of this date, not an earlier training-era year.",
		...
		toolSearchOnly:     "- Answer directly when search is unnecessary. Otherwise use aivory_web_search up to three times, batching independent queries in queries when useful. If results are missing or insufficient, refine the query for a later call; do not repeat identical searches. Stop as soon as the evidence is sufficient. After three calls, answer from the available results without further tools and state any remaining uncertainty.\n",
	...
	"zh": {
		...
		dateGrounding:      "今天是 %s。“今天”“现在”“最新”“近期”以及网络搜索查询中的日期都以此为准，不要沿用训练数据里更早的年份。",
		...
		toolSearchOnly:     "- 无需搜索时直接回答；需要时最多调用三次 aivory_web_search，多个独立查询可合并到 queries 中批量搜索。结果为空或不足时，可调整关键词再次搜索，不要重复相同查询。证据足够时立即回答，不必用满三次；三次调用后依据已有结果回答，不再调用工具，缺少依据时说明不确定性，不要编造。\n",
	...
	"zh-Hant": {
		...
		dateGrounding:      "今天是 %s。「今天」「現在」「最新」「近期」以及網路搜尋查詢中的日期都以此為準，不要沿用訓練資料裡更早的年份。",
		...
		toolSearchOnly:     "- 無需搜尋時直接回答；需要時最多呼叫三次 aivory_web_search，多個獨立查詢可合併到 queries 中批次搜尋。結果為空或不足時，可調整關鍵字再次搜尋，不要重複相同查詢。證據足夠時立即回答，不必用滿三次；三次呼叫後依據已有結果回答，不再呼叫工具，缺少依據時說明不確定性，不要編造。\n",
	...
	"ja": {
		...
		dateGrounding:      "今日は %s です。「今日」「今」「最新」「最近」やウェブ検索クエリ内の日付は、この日付を基準にし、訓練データの古い年は使わないでください。",
		...
		toolSearchOnly:     "- 検索が不要なら直接回答してください。必要なら aivory_web_search を最大3回使い、独立した検索語は queries にまとめてください。結果が空または不十分なら検索語を調整し、同じ検索を繰り返さないでください。十分な根拠が得られた時点で回答し、3回を使い切る必要はありません。3回後は追加のツールを使わず、確認できない点を明示して回答してください。\n",
	...
	"fr": {
		...
		dateGrounding:      "Nous sommes le %s. Rapporte « aujourd'hui », « dernier » et « récent » — ainsi que les dates de tes recherches web — à cette date, pas à une année antérieure de ton entraînement.",
		...
		toolSearchOnly:     "- Réponds directement si aucune recherche n’est nécessaire. Sinon, utilise aivory_web_search au maximum trois fois et regroupe les requêtes indépendantes dans queries. Si les résultats sont vides ou insuffisants, affine la requête sans répéter une recherche identique. Réponds dès que les preuves suffisent. Après trois appels, réponds sans autre outil et signale les incertitudes restantes.\n",
