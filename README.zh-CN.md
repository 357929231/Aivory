...
| **AI PPT 创作台** | 管理员接入文多多 Docmee、设定每份 PPT 的积分价格后，登录用户即可安全地创建演示文稿。 |
...
Aivory 支持开放标准的 **Model Context Protocol（Streamable HTTP 传输）**，通过“能力”方式接入外部工具：

- **管理员集中目录**：在「管理后台 → 能力与集成 → MCP 服务」注册一次即可复用。名称、图标、描述公开；服务地址与可选请求头（可携带 `Authorization: Bearer …` 等凭据）只留在服务端，保存后以掩码显示。
- **按模型默认**：模型记录可以预勾选其 MCP 工具（来自整个目录），用户仍可在单条对话内调整。
- **用户自管 MCP**：普通用户也可以在自己的资源库中添加 Streamable HTTP MCP 端点，与管理员目录相互独立。
- **接入前核验**：每个服务先测试再同步，发现的工具写入模型注册表使用的快照；标记不可用的服务仍会出现在选择器中但不可选（与受限内置工具同一规则）。
- **安全边界**：MCP 工具输出一律作为不可信数据处理（从不当作指令），与对话内工具调用相同的上限与作用域规则生效。
...
| 登录与 SSO | 邮箱/密码开关、注册策略、验证码、**OAuth/OIDC**（Google · GitHub · Apple · 通用 OIDC/OAuth2——Azure AD、Okta、Keycloak 走通用 OIDC）、TOTP 双因子、会话撤销 |
...
| MCP 工具集成 | 通过 Streamable HTTP MCP 开放接入标准工具——管理员目录 + 按模型默认 + 用户自管端点 |
...
- **五种登录模式**：Google、GitHub、Apple、**通用 OAuth 2.0**（UserInfo）、**通用 OpenID Connect**（经 JWKS 验签 ID Token）。凡支持 OIDC/OAuth 2 的 IdP——Azure AD / Entra ID、Okta、Keycloak、Auth0、GitLab、飞书/Lark、自建 IdP——都能通过通用类型填入其 authorize / token / userinfo / issuer / JWKS 接入。
- **企业 SSO 姿态**：关闭邮箱/密码注册与站点密码登录，只保留 OIDC/OAuth2 来源，并把未登录入口设为自动跳转到默认服务商。身份绑定以“subject 键控”加固——重登按服务商不可变 subject 匹配，而不是用户填写的邮箱；state nonce + PKCE 防回调重放。
- **开户与生命周期**：第三方登录可自动开户；账户可被策略要求为 OAuth 用户设置首次密码。管理员可集中绑定/解绑身份、轮换凭据、撤销会话。
- **安全默认**：OAuth 客户端密钥与 Apple `.p8` 只留在服务端；token 交换有界超时并给出清晰的出网诊断；OAuth 出入口均有 IP 限速；TOTP 双因子与登录审计同样作用于 SSO 会话。
...
可选的 **AI PPT** 创作台在「**管理后台 → 积分与配额 → AI PPT（文多多 Docmee）**」中配置。管理员保存 Docmee API Key 并打开启用开关后，登录用户的侧边栏才会显示 **AI PPT**；未配置或已关闭时入口保持隐藏，`/ppt` 页面也不能发起生成。配置步骤、积分计费、国际版与代理部署请阅读[《AI PPT（文多多 Docmee）使用与配置》](docs/ai-ppt-docmee.zh-CN.md)。
