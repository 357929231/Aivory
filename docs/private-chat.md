...
隐私对话由系统用户组与工作空间的“隐私对话”开关共同控制，只有两者都允许时才能使用。关闭后前端不显示入口，直接访问页面会返回首页，服务端也会拒绝 `POST /api/private-chat`。工作空间管理员和所有者仍受这两层权限限制；系统管理员沿用用户组豁免，但仍遵守工作空间开关。访客不能发起隐私对话。这个开关独立于普通对话的“仅自己可见”成员权限。

企业域名的“锁定个人空间”不会自动关闭隐私对话。锁定账号必须遵守所属工作空间权限；旧客户端省略 `workspace_id` 时，后端自动使用其指定工作空间，伪造其他空间会被拒绝。个人空间仅受用户组权限限制。新增工作空间开关及数据库迁移默认开启，保留升级前行为。
...
- 隐私页按当前工作空间过滤模型，服务端校验模型范围、图片上传开关和成员月度额度。仍使用个人账户计费，不创建工作空间会话，也不绑定项目、知识库或普通会话。用量记录只增加所属工作空间，用于统计和月度额度校验；工作空间管理员仍无法查看隐私对话正文。
...
- 只有模型启用 `vision`、用户组与当前工作空间都允许文件上传时，才显示图片选择按钮；服务端每次发送重新校验。对话中已有图片时，前端禁止切到非识图模型，避免静默丢失上下文。
...
`workspace_id` 在工作空间内发送，个人空间省略；它只用于权限校验。`messages` 必须从 user 开始、按 user/assistant 交替、以 user 结束。多轮请求由客户端携带内存中的历史，不返回或接受持久会话 ID、文件 URL、工具记录或服务商原始状态。思考内容不作为后续轮次的历史重放。
