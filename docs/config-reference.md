...
>
> **这 321 个环境变量全部是可选的，未在 `.env.example` 中列出。** 不设置任何一个，Aivory 使用下表所列的默认值。只有当你需要调整某个具体参数时，才在部署环境里添加对应的变量。
> 如果需要调整，把用到的变量抄一份加进你自己的 `.env` 即可（`.env.example` 保持精简，不会自动包含这些高级选项）。
>
> **后端（Go）**：292 个，读取自进程环境变量，改动后**需重启 `aivory-api` 进程**生效。
...
| `AIVORY_LLM_OFFICIAL_TOOL_SPEC` | `string` | `"medium"` | `store/official_tools.go` | `search_context_size` used only when converting a legacy OpenAI Responses `"web_search"` string entry to the current object format. New models receive no provider-hosted tools by default; explicit admin request JSON is stored unchanged. |
...
| `AIVORY_TOOLS_IN_SIZE` | `string` | `""` | `tools/builtins.go:51` | Optional image size override used when an image_generate call omits size. Empty lets the provider choose automatically; GPT image edits infer the closest supported ratio when possible. |
...
| `AIVORY_TOOLS_FETCHREMOTEIMAGE_DOWNLOAD_CAP` | `int64` | `32<<20` | `tools/builtins.go:48` | Byte cap for fetch_image public downloads and image URLs returned in image-API responses. Both paths use the SSRF-safe client and require verified image bytes. |
...
| `DOCMEE_API_KEY` | `string` | `""` | `api/docmee_handlers.go` | Optional fallback for the Docmee AI PPT API key when the administrator has not stored `docmee_api_key` in the database. The database setting takes precedence. |
| `DOCMEE_API_BASE_URL` | `string` | `"https://docmee.cn"` | `api/docmee_handlers.go` | Optional fallback for the server-side Docmee token API origin. The database setting takes precedence. |
| `DOCMEE_DOMAIN` | `string` | `""` | `api/docmee_handlers.go` | Optional fallback for the Docmee iframe domain, for example an international endpoint or an administrator-operated proxy. The database setting takes precedence. |
| `DOCMEE_SDK_URL` | `string` | pinned Docmee jsDelivr SDK URL | `api/docmee_handlers.go` | Optional fallback for the browser SDK script URL. Use it only for a self-hosted copy or trusted internal mirror when the database setting is blank. |
| `DOCMEE_SDK_BASE_URL` | `string` | `""` | `api/docmee_handlers.go` | Optional fallback for the SDK base URL used with a Docmee proxy. The database setting takes precedence. |
